...
//        double dt = (lastImuT_opt < 0) ? (1.0 / imuFrequence) : (imuTime - lastImuT_opt);
    ...
    if (!imuQueImu.empty()) {
      ...
      for (int i = 0; i < (int) imuQueImu.size(); ++i) {
        ...
        double imuTime = ROS_TIME(thisImu);
        double dt = (lastImuQT < 0  || (imuTime - lastImuT_opt) <= 0) ? (1.0 / 500.0) : (imuTime - lastImuQT);
        if(dt <= 0) std::cout << dt << std::endl;
//        double dt = (lastImuQT < 0) ? (1.0 / imuFrequence) : (imuTime - lastImuQT);
  ...
  void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw) {
    ...
    double imuTime = ROS_TIME(&thisImu);
    double dt = (lastImuT_imu < 0  || (imuTime - lastImuT_opt) <= 0) ? (1.0 / 500.0) : (imuTime - lastImuT_imu);
    if(dt <= 0) std::cout << dt << std::endl;
